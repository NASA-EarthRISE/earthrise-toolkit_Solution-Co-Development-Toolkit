...
from .moderation import rate_limit
...
@rate_limit(settings_prefix="UPLOAD")
@staff_member_required
def upload_file(request):
    ...
    # Belt-and-suspenders: resolved path must be inside UPLOAD_DIR.
    if not os.path.realpath(save_path).startswith(
        os.path.realpath(UPLOAD_DIR) + os.sep
    ):
        return JsonResponse({"message": "Invalid filename."}, status=400)
    ...
    try:
        ...
        LOG.info(f"[upload] Store type: {store_type}  file: {safe_name}")

        docs = extract_text_and_chunk(save_path)
        LOG.info(f"[upload] Extracted {len(docs)} chunks from '{safe_name}'")

        if not docs:
            return JsonResponse(
                {"message": f"WARNING: '{safe_name}' produced 0 chunks — file may be empty or image-only."},
                status=400,
            )
        ...
        store.upsert(docs)
        LOG.info(f"[upload] Upsert complete for '{safe_name}'")
        ...
        return JsonResponse({"message": f"File '{safe_name}' ingested ({len(docs)} chunks) via {store_type}."})

    except Exception as e:
        tb = traceback.format_exc()
        LOG.error(f"[upload] ERROR ingesting '{safe_name}': {e}\n{tb}")
        return JsonResponse({"message": f"ERROR ingesting {safe_name}: {e}"}, status=500)
